...
등고선 생성과 DXF 조립이 CPU를 쓰기 때문에 **Hugging Face Spaces 를 권한다.**
Render 무료의 0.1 CPU 에서는 1분이면 끝날 작업이 5분 넘게 걸릴 수 있다.
...
**수치표고모델** — `dem/` 폴더는 저장소에 올리지 않으므로(용량) 배포판은 전지구
자료로 등고선을 만든다. 국토지리정보원 DEM 을 쓰려면 호스팅에 디스크를 붙여
`/app/dem` 에 마운트하고 파일을 올린 뒤 `POST /api/dem/rescan` 을 호출한다.
